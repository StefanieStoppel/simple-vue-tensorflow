...
<script>
  ...
  export default {
    ...
    computed: {
      minXValue() {
        return Math.min(...this.xValues, this.predictedPointCoordinates.x || 0)
      },
      minYValue() {
        return this.model.predict(tf.tensor2d([this.minXValue], [1, 1])).get(0, 0)
      },
      maxXValue() {
        return Math.max(...this.xValues, this.predictedPointCoordinates.x || 0)
      },
      maxYValue () {
        return this.model.predict(tf.tensor2d([this.maxXValue], [1, 1])).get(0, 0)
      },
      pointCoordinates() {
